		...
		if err == nil {
			if info.IsDir() {
				...
				return &Model{
					...
					statusMsg:     "DIR: " + startPath,
				}, explorerModel.ChangeRoot(startPath)
			} else {
				// It's a file
				...
				return &Model{
					...
					reviewer:      diff.NewReviewModel(),
					pendingFile:   absPath,
					statusMsg:     "FILE: " + absPath,
				}, explorerModel.ChangeRoot(dir)
			}
		} else {
			// Error stat-ing the path
			return &Model{
				explorerWidth: 30,
				showExplorer:  true,
				focus:         FocusExplorer,
			...
			}, explorerModel.ChangeRoot(".")
		}
	}

	// No path provided - load current directory
	cwd, _ := os.Getwd()
	return &Model{
		explorerWidth: 30,
		showExplorer:  true,
...
func (m *Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case explorer.RefreshCompleteMsg:
		...
		// Open pending file after explorer loads
		if m.pendingFile != "" {
			m.statusMsg = fmt.Sprintf("Opening pending file: %s", m.pendingFile)
			cmds = append(cmds, m.editor.OpenFile(m.pendingFile))
			m.pendingFile = "" // Clear pending file
		}
	}
